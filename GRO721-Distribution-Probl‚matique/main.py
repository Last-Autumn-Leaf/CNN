...
class ConveyorCnnTrainer():
    ...
    def _create_model(self, task):
        if task == 'classification':
            return models.ClassificationNetwork(1,3)
        elif task == 'detection':
            # À compléter
            return models.DetectionNetwork(1,3)
        elif task == 'segmentation':
            # À compléter
            return models.SegmentationNetwork(1,4)
        else:
    ...
    def _create_criterion(self, task):
        if task == 'classification':
            ...
            # À compléter
            return models.DetectionNetworkLoss()
        elif task == 'segmentation':
            # À compléter
    ...
    def _train_batch(self, task, model, criterion, metric, optimizer, image, segmentation_target, boxes, class_labels):
        ...
        if task == "classification":
            ...
            #output=torch.reshape(output, (output.shape[0],3, 7))

            metric.accumulate(output, boxes)
        elif task == "segmentation":
            #output =torch.argmax(output, dim=1).to(torch.float)

            segmentation_target=segmentation_target.to(torch.float)
            a,b,c=segmentation_target.shape

            seg=torch.zeros(a,4,b,c)
            for i in range(len(segmentation_target)) :
                for j in range(len(segmentation_target[i])):
                    for k in range(len(segmentation_target[i,j])):
                        x=segmentation_target[i,j,k]
                        seg[i,int(x),j,k]=1
